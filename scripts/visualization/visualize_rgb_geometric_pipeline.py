"""Visualize RGB-Geometric Pipeline: Original -> YOLO Crop -> Final with GT."""
...
def draw_label_with_bg(img, text, x, y, color, scale=0.55, thickness=1):
    ...
    y = int(max(th + 5, min(y, img.shape[0] - 10)))
    cv2.rectangle(img, (x - 3, y - th - 4), (x + tw + 3, y + 2), (0, 0, 0), -1)
    cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, scale, color, thickness, cv2.LINE_AA)


def run_pipeline_visualization(img_path):
    """
    Visualize the RGB-Geometric pipeline in 3 stages:
    1. Original image
    2. YOLO crop with detected bounding box
    3. Final result with predicted (cyan) and GT (green) 3D bounding boxes
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"RGB-Geometric Pipeline Visualization on {device}")
    ...
    # Prepare figure with 3 subplots per detection (or show first detection only)
    num_detections = len(results[0].boxes)
    
    # For simplicity, visualize only the first detection
    ...
    stage1_img = original_img.copy()
    # put_text_with_outline(stage1_img, "Original Image", (10, 30), (255, 255, 255), scale=0.8, thickness=2)
    
    # ============ STAGE 2: YOLO Crop with Bbox ============
    # Draw YOLO bounding box
    stage2_img = original_img.copy()
    cv2.rectangle(stage2_img, (x1, y1), (x2, y2), (221, 240, 48), 3)  # yellow bbox
    
    # Show crop region with padding
    ...
    # Draw expanded crop region
    crop_x1 = max(0, new_x)
    crop_y1 = max(0, new_y)
    crop_x2 = min(w_img, new_x + new_size)
    crop_y2 = min(h_img, new_y + new_size)
    # cv2.rectangle(stage2_img, (crop_x1, crop_y1), (crop_x2, crop_y2), (0, 255, 255), 2)  # Cyan crop region
    
    # Add labels
    draw_label_with_bg(stage2_img, f"YOLO: {obj_id_str} ({conf:.2f})", x1, y1 - 10, (255, 255, 255), scale=0.6)
    # put_text_with_outline(stage2_img, "YOLO Detection + Crop Region", (10, 30), (255, 255, 255), scale=0.8, thickness=2)
    # put_text_with_outline(stage2_img, "Yellow=Detection | Yellow=Crop", (10, 60), (255, 255, 255), scale=0.6, thickness=2)
    ...
    if gt_rot is not None and gt_trans is not None and obj_id_str == file_obj_id:
        ...
        if model_points is not None:
            ...
            print(f"\n  Object {obj_id_str} Results:")
            print(f"    ADD Error:   {metrics['add_mm']:.1f}mm")
    ...
    if corners is not None:
        ...
        draw_3d_box(stage3_img, box_2d, (0, 255, 255), 3)
        draw_axes(stage3_img, pred_quat, pred_trans, K, scale=0.1)
        ...
        if gt_rot is not None and gt_trans is not None:
            ...
            draw_3d_box(stage3_img, gt_box_2d, (0, 255, 0), 2)
    
    # Add title and metrics
    put_text_with_outline(stage3_img, "6D Pose Estimation Result", (10, 30), (255, 255, 255), scale=0.8, thickness=2)
    
    if metrics is not None:
        add_mm = metrics['add_mm']
        trans_err = metrics['trans_error_mm']
        rot_err = metrics['rot_error_deg']
        ...
        put_text_with_outline(stage3_img, f"ADD: {add_mm:.1f}mm | Trans: {trans_err:.1f}mm | Rot: {rot_err:.1f}deg", 
                            (10, 60), color, scale=0.7, thickness=2)
    
    put_text_with_outline(stage3_img, "Yellow=Predicted | Green=GT", (10, h_img - 70), (255, 255, 255), scale=0.6, thickness=2)
    put_text_with_outline(stage3_img, "Axes: X=Red | Y=Green | Z=Blue", (10, h_img - 40), (255, 255, 255), scale=0.6, thickness=2)
    
    # ============ Display All Stages ============
    fig, axes = plt.subplots(1, 3, figsize=(20, 7))
    
    # Stage 1: Original
    axes[0].imshow(cv2.cvtColor(stage1_img, cv2.COLOR_BGR2RGB))
    axes[0].set_title("Stage 1: Original Image", fontsize=14, fontweight='bold')
    axes[0].axis('off')
    
    # Stage 2: YOLO Detection
    axes[1].imshow(cv2.cvtColor(stage2_img, cv2.COLOR_BGR2RGB))
    axes[1].set_title("Stage 2: YOLO Detection + Crop", fontsize=14, fontweight='bold')
    axes[1].axis('off')
    
    # Stage 3: Final Result
    axes[2].imshow(cv2.cvtColor(stage3_img, cv2.COLOR_BGR2RGB))
    axes[2].set_title("Stage 3: 6D Pose (Pred vs GT)", fontsize=14, fontweight='bold')
    axes[2].axis('off')
    
    plt.tight_layout()
    plt.suptitle(f"RGB-Geometric Pipeline: {os.path.basename(img_path)}", 
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if os.path.exists(TEST_DIR):
            ...
            if len(files) > 0:
                ...
                print(f"Selected random test image: {random_file}")
            else:
    ...
    run_pipeline_visualization(TEST_IMG)
