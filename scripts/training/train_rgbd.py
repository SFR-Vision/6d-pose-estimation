...
def train():
    ...
    if start_epoch > 0 and os.path.exists(history_path):
        ...
        history = {'train_loss': [], 'val_loss': [], 'val_add': [], 'val_acc_2cm': [], 'lr': []}
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for rgbdm, z_sensor, gt_rot, gt_trans, obj_ids, bbox_center, cam_matrix in pbar:
        ...
        with torch.no_grad():
            for rgbdm, z_sensor, gt_rot, gt_trans, obj_ids, bbox_center, cam_matrix in val_loader:
                rgbdm = rgbdm.to(DEVICE)
                z_sensor = z_sensor.to(DEVICE)
                gt_rot = gt_rot.to(DEVICE)
                gt_trans = gt_trans.to(DEVICE)
        ...
        print(f"  Learned weights - Rot: {weights['rot_weight']:.3f}, Trans: {weights['trans_weight']:.3f} (σ_rot: {weights['sigma_rot']:.3f}, σ_trans: {weights['sigma_trans']:.3f})")
