...
def load_mesh_data(obj_id_str):
    """Loads mesh and calculates bounding box"""
...
def get_gt_and_K(obj_id_str, frame_id):
    """Load ground truth pose and camera intrinsics"""
...
def visualize():
    ...
    print("🚀 Running RGB-D Visualization...")
    ...
    depth_path = os.path.join(ORIG_DATA_DIR, obj_id_str, "depth", f"{str(frame_id).zfill(4)}.png")
    depth_img = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
    if depth_img is None:
        print(f"⚠️ Warning: Depth image not found at {depth_path}")
    ...
    # Normalize depth
    crop_depth_normalized = crop_depth_resized.astype(np.float32) / 1000.0  # mm to meters
    ...
    if box_corners_3d is not None:
        ...
        # Ground Truth Box (Green - dashed style)
        if gt_r is not None:
            box_2d_gt = project_points(box_corners_3d, gt_r, gt_t, K)
            # Draw dashed lines for ground truth
            lines = [(0,1), (1,2), (2,3), (3,0), (4,5), (5,6), (6,7), (7,4), (0,4), (1,5), (2,6), (3,7)]
            for s, e in lines:
                pt1, pt2 = tuple(box_2d_gt[s]), tuple(box_2d_gt[e])
                # Dashed line effect
                draw_dashed_line(viz_img, pt1, pt2, (0, 255, 0), 1, dash_length=5)
        
        # Prediction Box (Yellow - solid)
        box_2d_pred = project_points(box_corners_3d, pred_rot, corrected_trans, K)
        draw_box(viz_img, box_2d_pred, (0, 255, 255), 2)
        
        # Draw orientation axes for PREDICTION
        center_pt = project_points(np.array([[0,0,0]]), pred_rot, corrected_trans, K)[0]
        axis_length = 0.08
        x_pt = project_points(np.array([[axis_length,0,0]]), pred_rot, corrected_trans, K)[0]
        y_pt = project_points(np.array([[0,axis_length,0]]), pred_rot, corrected_trans, K)[0]
        z_pt = project_points(np.array([[0,0,axis_length]]), pred_rot, corrected_trans, K)[0]
        
        # Axis lines with labels
        cv2.line(viz_img, tuple(center_pt), tuple(x_pt), (0, 0, 255), 3)    # X = Red
        cv2.line(viz_img, tuple(center_pt), tuple(y_pt), (0, 255, 0), 3)    # Y = Green  
        cv2.line(viz_img, tuple(center_pt), tuple(z_pt), (255, 0, 0), 3)    # Z = Blue
        
        # Circles at endpoints
        cv2.circle(viz_img, tuple(x_pt), 5, (0, 0, 255), -1)
        cv2.circle(viz_img, tuple(y_pt), 5, (0, 255, 0), -1)
        cv2.circle(viz_img, tuple(z_pt), 5, (255, 0, 0), -1)
        
        # Axis labels (small, clean)
        cv2.putText(viz_img, "X", tuple(x_pt + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.putText(viz_img, "Y", tuple(y_pt + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        cv2.putText(viz_img, "Z", tuple(z_pt + 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

    # Get object name with ID
    obj_name = OBJECT_NAMES.get(obj_id_str, "Unknown")
    title_text = f"{obj_name} (ID: {obj_id_str})"
    
    plt.figure(figsize=(14, 11))
    plt.imshow(cv2.cvtColor(viz_img, cv2.COLOR_BGR2RGB))
    plt.title(f"6D Pose Estimation - {title_text}\n" + 
              "Green Dashed = Ground Truth | Yellow Solid = Prediction\n" +
              "Axes (Prediction): Red=X (Forward), Green=Y (Left), Blue=Z (Up)", 
              fontsize=14, pad=15)
    plt.axis("off")
    plt.show()
